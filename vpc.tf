...
# Private application-tier subnets - host the frontend autoscaling group
resource "aws_subnet" "app" {
  ...
  tags = merge(local.common_tags, {
    Name = "${local.name_prefix}-app-${local.azs[count.index % length(local.azs)]}"
    Tier = "app"
  })
}

# Private data-tier subnets - host the backend EC2 instances
resource "aws_subnet" "data" {
  ...
  tags = merge(local.common_tags, {
    Name = "${local.name_prefix}-data-${local.azs[count.index % length(local.azs)]}"
    Tier = "data"
  })
}
